<template>
  <b-card>
      ...
      >
        <template v-slot:cell(index)="row">{{ row.index + 1 }}</template>
          <template #cell(voucher)="data">
            <div class="d-flex flex-wrap justify-content-between align-items-center" style="width: 100px">
              <div  v-for="(imagen) in data.item.imagenes" :key="imagen">
                <b-tooltip :target="`imagen-${imagen}`" triggers="hover" variant="light" v-if="imagen != 'default.jpg'">
                  <b-button size="sm" @click="showImageClick(`/uploads/vouchers_por_pedido/${imagen}`)" variant="info">
                    <i class="icon-eye"></i>
                  </b-button>
                  <b-button size="sm" @click="eliminarFoto(imagen)" variant="danger">
                    <i class="icon-trash"></i>
                  </b-button>
                </b-tooltip>
                <template v-if="data.item.imagenes.length > 1">
                  <b-img :id="`imagen-${imagen}`" center :src="`/uploads/vouchers_por_pedido/${imagen}`" :class="(imagen!='default.jpg'?'icono-upload':'')" style="width: 30px" v-if="imagen != 'default.jpg'"></b-img>
                </template>
                <template v-if="data.item.imagenes.length <= 1">
                  <b-img :id="`imagen-${imagen}`" center :src="`/uploads/vouchers_por_pedido/default.jpg`" :class="(imagen!='default.jpg'?'icono-upload':'')" style="width: 30px"></b-img>
                </template>
              </div>
          ...
          <template #cell(por_cobrar)="data">
            {{((Number(data.item.precio_productos)+Number(data.item.envio_productos))-Number(data.item.monto_pagado)).toFixed(2)}}
        ...
        <template v-slot:cell(actions)="row">
          <div>
            ...
            <b-button size="sm" variant="danger" @click="anularPedido(row.item, row.index)" v-if="usuarioLogeado.idrole != 4">
                <i class="icon-trash"></i>
              </b-button>
            <!--<b-button
              variant="danger"
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <button type="button" class="close" @click="cerrarModal('#modalSubirImagen')" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <vue-dropzone ref="myVueDropzone" id="dropzone" :useCustomSlot="true" :options="dropzoneOptions" v-on:vdropzone-sending="sendingEvent">

              <div class="dropzone-custom-content">
                <h3 class="dropzone-custom-title">Arrastra y suelta aquí el voucher!</h3>
                <div class="subtitle">...o has click aquí para seleccionar la foto desde tu computadora</div>
              </div>
            </vue-dropzone>
          </div>
        </div>
    ...
    <div class="modal fade bd-example-modal-lg1" id="modalEnvioEditar" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" ref="modalEnvioEditar" style="overflow-y: scroll;">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Editar pedido</h5>
                            <button type="button" class="close" @click="cerrarModal('#modalEnvioEditar')" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="card-body">
                            <form  action="" @submit.prevent="editarPedido()">
                                ...
                                <div class="form-row">
                                  <template v-if="usuarioLogeado.idrole != 4">
                                    ...
                                    <div class="form-group col-md-3">
                                        <label>Fecha</label>
                                        <input type="date" name="FechaReg"  id="FechaReg_edit" class="form-control" v-model="objetoEnvio.fecha">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label>Cliente</label>
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <input type="number" step="any" class="form-control" id="precio_productos_edit" v-model="objetoEnvio.precio_productos">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label>Costo de envío</label>
                                        <input type="number" step="any" class="form-control" id="costo_envio_edit" v-model="objetoEnvio.envio_productos">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label>Monto pagado</label>
                                        <input type="number" step="any" class="form-control" id="monto_pagado_edit" v-model="objetoEnvio.monto_pagado">
                                    </div>
                                    <div class="form-group col-md-3">
                                ...
                                <hr>
                                 <div class="form-group" v-if="usuarioLogeado.idrole != 4">
                                        <label>Pedido</label>
                                        <textarea class="form-control" rows="3" v-model="objetoEnvio.pedido"></textarea>
                                    </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" @click="cerrarModal('#modalEnvioEditar')">Cerrar</button>
              ...
              <div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" id="modalDetallePedido" aria-labelledby="myLargeModalLabel" aria-hidden="true">
...
export default {
  ...
  mounted(){
    $(this.$refs.modalEnvioEditar).on("hidden.bs.modal", this.limpiarModalEditarEnvio)
  ...
  methods: {
    ...
    anularPedido(item, index){
            Vue.swal({
                title: 'Advertencia',
                text: '¿Estás seguro de anular este pedido? !Esta acción no se puede deshacer!',
                icon: 'warning'
            }).then((result) => {
                if (result.value) {
                    this.$store.dispatch('actualizarShow', true)
                    axios.put(`delivery/${item.id}/anularPedido`).then(response=>{
    ...
    obtenerPedidosPorDia(){
        this.cargando = true
            axios.get(`/delivery/${this.dia.fecha}/obtenerPedidosPorFecha`).then(response=>{
    ...
    eliminarFoto(nombreFotoVoucher){
      if(confirm(`Deseas eliminar la foto ${nombreFotoVoucher}?`)){
        axios.delete(`/voucher_pedido/${nombreFotoVoucher}`).then(response=>{
          this.obtenerPedidosPorDia()
        })
      }
    },
    ...
    abrirModalEditarPedido(pedido){
      /*if(this.usuarioLogeado.idrole == 4 && pedido.estado == 'pagado'){
        Vue.swal({
                title: 'Advertencia',
                text: 'No puedes editar un pedido pagado',
                icon: 'warning'
            })
      }else{*/
        this.pedidoSeleccionado = pedido
    ...
    editarPedido(){
      this.$store.dispatch('actualizarShow', true)
        axios.put(`delivery/${this.pedidoSeleccionado.id}`, this.objetoEnvio).then(response=>{
          this.$store.dispatch('actualizarShow', false)
