          ...
          <b-card-body>
            <form @submit.prevent="obtenerResultadosFiltro()">
              <div class="form-group row">
                ...
                <div class="col-sm-5">
                  <select v-model="categoria_filtro" class="form-control">
                    <option :value="0">Escoje una categoría</option>
                    <option
  ...
  methods: {
    ...
    obtenerResultadosFiltro() {
      ...
      if (!this.filtrarPorRango) {
        axios
          .get(
            `d_fact/${this.fecha_inicio_filtro}/${this.almacen_filtro}/${this.categoria_filtro}/rankingProductosCategoria`
          )
          .then(response => {
        ...
        axios
          .get(
            `d_fact/${this.fecha_inicio_filtro}/${this.fecha_fin_filtro}/${this.almacen_filtro}/${this.categoria_filtro}/rankingProductosPorRangoCategoria`
          )
          .then(response => {
            this.arrayReporte = response.data;
            this.totalRows = this.arrayReporte.length;
